<template>
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="(item, index) in courses" :key="index">
                    ...
                    <td>
                      <span class="editable" @click="editCell(index, 'faculty')">{{ item.faculty }}</span>
                  </td>
                  
                  <td>
                      <span class="editable" @click="editCell(index, 'department')">{{ item.department }}</span>
                  </td>
                  <td>
  ...
  export default {
    ...
    methods: {
      ...
      addCourse() {
        this.courses.push({ faculty: "Mühendislik", department: "Bilgisayar Mühendisliği", courseName: "Ders Adı", akts: 0, credit: 0 });
      },
      logoutUser() {
