<template>
            ...
            <table class="table">
              ...
              <tbody>
                <tr v-for="(item, index) in programs" :key="item.id">
                  <td>{{ 'PÇ ' + (index + 1) }}</td>
                  <td>
                    <span v-if="!item.editableDescription">{{ item.description }}</span>
                  ...
                  <td>
                    ...
                    <button v-if="!item.editableDescription" class="btn btn-warning btn-sm ml-2 text-white" @click="editProgram(item)">Düzenle</button>
  ...
  export default {
    ...
    methods: {
      ...
      editProgram(item) {
        // Program Çıktısı Açıklaması sütununu düzenlenebilir hale getir
        item.editableDescription = true;
      },
      saveProgram(index) {
        ...
        axios.put(`http://localhost:8080/program-outcomes/${program.id}`, {
          ...
          .then(response => {
            ...
            program.editableDescription = false;
          })
          .catch(error => {
